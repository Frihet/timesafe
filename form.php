...
class form
{
    ...
    function makeSelect($name, $values, $selected=null, $id=null, $attributes=array()) {
        ...
        $name_str = htmlEncode($name) . ($multiple?'[]':'');
        $select = "<select $attribute_str name='$name_str' $id_str>";
...
";
	...
	function makeHidden($name, $value, $id=null) 
	{
            $id_str = 'id="' . htmlEncode($id?$id:$name) . '"';
            return "<input type='hidden' $id_str name='".htmlEncode($name)."' value='".htmlEncode($value)."'>\n";		
	}
